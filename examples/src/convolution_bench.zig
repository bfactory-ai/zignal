...
fn benchOp(io: std.Io, name: []const u8, rows: usize, cols: usize, filter: ?[]const u8, ctx: anytype) !void {
    if (filter) |f| {
        if (std.mem.indexOf(u8, name, f) == null) return;
    }
...
fn benchGaussian(comptime T: type, io: std.Io, gpa: std.mem.Allocator, random: std.Random, filter: ?[]const u8, rows: usize, cols: usize, sigma: f32) !void {
    var src: Image(T) = try .init(gpa, @intCast(rows), @intCast(cols));
    defer src.deinit(gpa);
    var dst: Image(T) = try .init(gpa, @intCast(rows), @intCast(cols));
    ...
    try benchOp(io, name, rows, cols, filter, Ctx{ .src = src, .dst = dst, .gpa = gpa, .sigma = sigma });
}
...
fn benchSobel(io: std.Io, gpa: std.mem.Allocator, random: std.Random, filter: ?[]const u8, rows: usize, cols: usize) !void {
    ...
    try benchOp(io, "sobel f32", rows, cols, filter, Ctx{ .src = src, .dst = dst, .gpa = gpa });
}
...
fn benchMotionBlur(comptime T: type, io: std.Io, gpa: std.mem.Allocator, random: std.Random, filter: ?[]const u8, rows: usize, cols: usize, distance: usize) !void {
    ...
    try benchOp(io, name, rows, cols, filter, Ctx{ .src = src, .dst = dst, .gpa = gpa, .distance = distance });
}
...
fn benchSeparable(comptime T: type, io: std.Io, gpa: std.mem.Allocator, random: std.Random, filter: ?[]const u8, rows: usize, cols: usize) !void {
    ...
    try benchOp(io, name, rows, cols, filter, Ctx{ .src = src, .dst = dst, .gpa = gpa });
}
