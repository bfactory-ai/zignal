...
pub fn encode(allocator: Allocator, image_data: []const u8, width: u32, height: u32, color_type: ColorType, bit_depth: u8) ![]u8 {
    ...
    // Compress filtered data with zlib format (required for PNG IDAT) using uncompressed for now
    // TODO: Fix static Huffman implementation
    const compressed_data = try deflate.zlibCompress(allocator, filtered_data, .uncompressed);
