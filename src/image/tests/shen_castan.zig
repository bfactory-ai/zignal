...
test "Shen-Castan edge detection basic functionality" {
    ...
    try filter.shenCastan(img, io, allocator, edges, opts);
...
test "Shen-Castan parameter validation" {
    ...
    try testing.expectError(error.InvalidBParameter, filter.shenCastan(img, io, allocator, edges, .{ .smooth = 0.0 }));
    try testing.expectError(error.InvalidBParameter, filter.shenCastan(img, io, allocator, edges, .{ .smooth = 1.0 }));
    try testing.expectError(error.InvalidBParameter, filter.shenCastan(img, io, allocator, edges, .{ .smooth = -0.5 }));

    // Test invalid thresholds (ratio-based)
    try testing.expectError(error.InvalidThreshold, filter.shenCastan(img, io, allocator, edges, .{ .high_ratio = 0.0 }));
    try testing.expectError(error.InvalidThreshold, filter.shenCastan(img, io, allocator, edges, .{ .high_ratio = 1.0 }));
    try testing.expectError(error.InvalidThreshold, filter.shenCastan(img, io, allocator, edges, .{ .low_rel = 0.0 }));

    // Test invalid window size (even number)
    try testing.expectError(error.WindowSizeMustBeOdd, filter.shenCastan(img, io, allocator, edges, .{ .window_size = 6 }));
...
test "Shen-Castan with different b parameters" {
    ...
    try filter.shenCastan(img, io, allocator, edges_low_b, .{ .smooth = 0.7, .window_size = 7, .high_ratio = 0.9 });

    // Test with high b (less smoothing)
    var edges_high_b = try Image(u8).initLike(allocator, img);
    defer edges_high_b.deinit(allocator);
    try filter.shenCastan(img, io, allocator, edges_high_b, .{ .smooth = 0.9, .window_size = 7, .high_ratio = 0.9 });
...
test "Shen-Castan on RGB image" {
    ...
    try filter.shenCastan(img, io, allocator, edges, .{ .smooth = 0.8, .window_size = 7, .high_ratio = 0.9 });
...
test "Shen-Castan threshold monotonicity" {
    ...
    try filter.shenCastan(img, io, allocator, edges_low, .{ .smooth = 0.8, .window_size = 7, .high_ratio = 0.95, .low_rel = 0.5 });

    // Test with high thresholds
    var edges_high = try Image(u8).initLike(allocator, img);
    defer edges_high.deinit(allocator);
    try filter.shenCastan(img, io, allocator, edges_high, .{ .smooth = 0.8, .window_size = 7, .high_ratio = 0.999, .low_rel = 0.5 });
...
test "Shen-Castan window size effect" {
    ...
    try filter.shenCastan(img, io, allocator, edges_small, .{ .smooth = 0.8, .window_size = 3, .high_ratio = 0.9 });
    ...
    try filter.shenCastan(img, io, allocator, edges_large, .{ .smooth = 0.8, .window_size = 11, .high_ratio = 0.9 });
