//! 8x8 font data, originally generated from the font8x8 library
//!
...
pub fn findCharData(codepoint: u21) ?[]const u8 {
    for (ranges) |range| {
        if (codepoint >= range.start and codepoint <= range.end) {
            const offset = (codepoint - range.start) * 8;
            if (offset + 8 <= range.data.len) {
                return range.data[offset .. offset + 8];
            }
        }
    }
    return null;
