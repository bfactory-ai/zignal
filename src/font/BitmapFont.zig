...
/// Returns an error if the file path doesn't end in `.bdf`, `.BDF`, `.bdf.gz`, or `.BDF.gz`.
pub fn save(self: BitmapFont, allocator: Allocator, file_path: []const u8) !void {
    const valid_extension = std.mem.endsWith(u8, file_path, ".bdf") or
        std.mem.endsWith(u8, file_path, ".BDF") or
        std.mem.endsWith(u8, file_path, ".bdf.gz") or
        std.mem.endsWith(u8, file_path, ".BDF.gz");
