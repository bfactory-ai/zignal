...
pub const image_difference_of_gaussians_doc =
    \\Apply Difference of Gaussians (DoG) filter to the image.
    \\
    \\Computes the difference between two Gaussian blurs with different sigmas.
    \\This is commonly used for edge detection and as an approximation of the
    \\Laplacian of Gaussian. The result highlights regions of rapid intensity
    \\change and is useful for feature enhancement and blob detection.
    \\
    \\## Parameters
...
pub fn image_difference_of_gaussians(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    const self = @as(*ImageObject, @ptrCast(self_obj.?));
    ...
    // Validate sigmas: must be finite and > 0
    if (!std.math.isFinite(sigma1) or !std.math.isFinite(sigma2)) {
        c.PyErr_SetString(c.PyExc_ValueError, "sigmas must be finite");
        return null;
    }

    if (sigma1 <= 0) {
        c.PyErr_SetString(c.PyExc_ValueError, "sigma1 must be positive (> 0)");
        return null;
    }
    if (sigma2 <= 0) {
        c.PyErr_SetString(c.PyExc_ValueError, "sigma2 must be positive (> 0)");
        return null;
    }

    const sigma1_pos = sigma1;
    const sigma2_pos = sigma2;

    // Check that sigmas are different
    if (sigma1_pos == sigma2_pos) {
        c.PyErr_SetString(c.PyExc_ValueError, "sigma1 and sigma2 must be different");
        return null;
    }
