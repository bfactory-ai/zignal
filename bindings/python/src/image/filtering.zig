...
pub fn image_box_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    return self.py_image.?.dispatch(.{radius}, struct {
        fn apply(img: anytype, r: u32) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).boxBlur, .{ img.*, python.io, allocator, out, @as(u32, @intCast(r)) }) catch {
                python.setMemoryError("image operation");
                return null;
            };
...
pub fn image_median_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    return self.py_image.?.dispatch(.{radius}, struct {
        fn apply(img: anytype, r: u32) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).medianBlur, .{ img.*, python.io, allocator, out, @as(usize, @intCast(r)) }) catch |err| {
                switch (err) {
...
pub fn image_min_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    return self.py_image.?.dispatch(.{ radius, border }, struct {
        fn apply(img: anytype, r: u32, b: zignal.BorderMode) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).minBlur, .{ img.*, python.io, allocator, out, @as(usize, @intCast(r)), b }) catch |err| {
                switch (err) {
                    error.InvalidRadius => python.setValueError("radius must be > 0", .{}),
...
pub fn image_max_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    return self.py_image.?.dispatch(.{ radius, border }, struct {
        fn apply(img: anytype, r: u32, b: zignal.BorderMode) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).maxBlur, .{ img.*, python.io, allocator, out, @as(usize, @intCast(r)), b }) catch |err| {
                switch (err) {
                    error.InvalidRadius => python.setValueError("radius must be > 0", .{}),
...
pub fn image_midpoint_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    return self.py_image.?.dispatch(.{ radius, border }, struct {
        fn apply(img: anytype, r: u32, b: zignal.BorderMode) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).midpointBlur, .{ img.*, python.io, allocator, out, @as(usize, @intCast(r)), b }) catch |err| {
                switch (err) {
                    error.InvalidRadius => python.setValueError("radius must be > 0", .{}),
...
pub fn image_percentile_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    return self.py_image.?.dispatch(.{ radius, percentile_value, border_mode }, struct {
        fn apply(img: anytype, r: u32, p: f64, b: zignal.BorderMode) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).percentileBlur, .{ img.*, python.io, allocator, out, @as(usize, @intCast(r)), p, b }) catch |err| {
                switch (err) {
                    error.InvalidRadius => python.setValueError("radius must be > 0", .{}),
                    error.InvalidPercentile => python.setValueError("percentile must be between 0 and 1", .{}),
                    error.UnsupportedPixelType => python.setValueError("percentile blur requires u8, RGB, or RGBA images", .{}),
                    else => python.setMemoryError("image operation"),
                }
                return null;
            };
...
pub fn image_alpha_trimmed_mean_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    if (!std.math.isFinite(params.trim_fraction)) {
        python.setValueError("trim_fraction must be finite", .{});
        return null;
    }
    ...
    return self.py_image.?.dispatch(.{ radius, params.trim_fraction, border }, struct {
        fn apply(img: anytype, r: u32, tf: f64, b: zignal.BorderMode) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).alphaTrimmedMeanBlur, .{ img.*, python.io, allocator, out, @as(usize, @intCast(r)), tf, b }) catch |err| {
                switch (err) {
                    error.InvalidRadius => python.setValueError("radius must be > 0", .{}),
                    error.InvalidTrim => python.setValueError("trim_fraction must be in [0, 0.5)", .{}),
                    error.UnsupportedPixelType => python.setValueError("alpha trimmed mean requires u8, RGB, or RGBA images", .{}),
                    else => python.setMemoryError("image operation"),
                }
                return null;
            };
...
pub fn image_gaussian_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    if (!std.math.isFinite(params.sigma)) {
        python.setValueError("sigma must be finite", .{});
        return null;
    }
...
pub fn image_sharpen(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    return self.py_image.?.dispatch(.{radius}, struct {
        fn apply(img: anytype, r: u32) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).sharpen, .{ img.*, python.io, allocator, out, @as(usize, @intCast(r)) }) catch {
                python.setMemoryError("image operation");
                return null;
            };
            return @ptrCast(moveImageToPython(out) orelse return null);
...
pub fn image_autocontrast(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    if (params.cutoff < 0 or params.cutoff >= 0.5) {
        python.setValueError("cutoff must be between 0 and 0.5", .{});
        return null;
    }
...
pub fn image_motion_blur(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    if (!std.mem.eql(u8, std.mem.span(type_name), "zignal.MotionBlur")) {
        python.setTypeError("MotionBlur object", config_obj);
        return null;
    }
    ...
    return self.py_image.?.dispatch(.{blur_obj}, struct {
        fn apply(img: anytype, bo: *motion_blur.MotionBlurObject) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).motionBlur, .{ img.*, python.io, allocator, out, blur_config }) catch {
                python.setMemoryError("image operation");
                return null;
            };
...
pub fn image_sobel(self_obj: ?*c.PyObject, args: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    return self.py_image.?.dispatch(.{}, struct {
        fn apply(img: anytype) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).sobel, .{ img.*, python.io, allocator, out }) catch {
                python.setMemoryError("image operation");
                return null;
            };
            return @ptrCast(moveImageToPython(out) orelse return null);
...
pub fn image_shen_castan(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    if (params.smooth <= 0 or params.smooth >= 1) {
        python.setValueError("smooth must be between 0 and 1", .{});
        return null;
    }
    if (params.window_size < 3) {
        python.setValueError("window_size must be >= 3", .{});
        return null;
    }
    if (@mod(params.window_size, 2) == 0) {
        python.setValueError("window_size must be odd", .{});
        return null;
    }
    ...
    return self.py_image.?.dispatch(.{opts}, struct {
        fn apply(img: anytype, o: zignal.ShenCastan) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).shenCastan, .{ img.*, python.io, allocator, out, o }) catch |err| {
...
pub const image_canny_doc =
    ...
    \\edges_sensitive = img.canny(high=90,sigma=1.0, low=30)
    \\
    \\# Conservative edge detection (higher thresholds)
    \\edges_conservative = img.canny(high=200,sigma=2.0, low=100)
    \\```
;
...
pub fn image_canny(self_obj: ?*c.PyObject, args: ?*c.PyObject, kwds: ?*c.PyObject) callconv(.c) ?*c.PyObject {
    ...
    if (!std.math.isFinite(params.low) or !std.math.isFinite(params.high)) {
        python.setValueError("thresholds must be finite", .{});
        return null;
    }

    const sigma = python.validateNonNegative(f32, params.sigma, "sigma") catch return null;
    const low = python.validateNonNegative(f32, params.low, "low") catch return null;
    const high = python.validateNonNegative(f32, params.high, "high") catch return null;

    if (low >= high) {
        python.setValueError("low must be less than high", .{});
        return null;
    }
    ...
    return self.py_image.?.dispatch(.{ sigma, low, high }, struct {
        fn apply(img: anytype, s: f32, l: f32, h: f32) ?*c.PyObject {
            ...
            python.withoutGil(@TypeOf(img.*).canny, .{ img.*, python.io, allocator, out, s, l, h }) catch |err| {
                if (err == error.InvalidParameter) {
                    python.setValueError("parameters must be finite numbers", .{});
                } else if (err == error.InvalidSigma) {
                    python.setValueError("sigma must be non-negative", .{});
                } else if (err == error.InvalidThreshold) {
                    python.setValueError("thresholds are invalid", .{});
                } else {
                    python.setMemoryError("image operation");
                }
                return null;
