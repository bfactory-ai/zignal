...
### Changed
- **JPEG encodes write a restart marker per MCU row by default**: `EncodeOptions.restart_interval` is a `RestartInterval` (`.none`, `.{ .rows = n }`, `.{ .mcus = n }`) defaulting to one row, so files from `save`/`encode` (and Python) decode in parallel. Cost: ~0.1 % of the file at 4K and ~7 % slower single-threaded decode (a DC predictor reset and a marker resync per row); `.none` restores the old output byte for byte.
