...
### Features
- **GPU compute (Vulkan + SPIR-V)**: new `zignal.gpu` namespace. Kernels are written in Zig and cross-compiled to SPIR-V by `build.zig` (`-Dgpu`, on by default except for freestanding targets); `gpu.Device` opens the Vulkan loader at runtime with `std.DynLib`, so zignal keeps its zero-dependency build and machines without a GPU get `error.GpuUnavailable` instead of a link failure (on Linux the calling module must link libc). First kernel: `Device.gemm`, a 16×16 shared-memory tiled f32 GEMM with the same arguments as `Matrix(f32).gemm`. On an Intel Arc iGPU, including upload and readback, it matches the blocked CPU kernel at 512² and is 2× faster at 1024² and 4.7× at 2048². `examples/src/gemm_bench.zig` (`zig build run-gemm-bench -Drelease=true -- 2048`) reproduces the numbers.
...
- **16-bit JPEG inverse DCT**: coefficient blocks are stored as `i16` (half the memory: 65 MB instead of 130 MB for a progressive 4K frame) and two adjacent blocks of a component are dequantized and transformed together in 16-lane vectors, the stb_image formulation of libjpeg's exact integer transform with paired multiply-adds and in-lane transposes, writing straight into the sample planes. Blocks past the image edge decode into the store instead of a scratch block. The scan-end search uses a vectorized `0xFF` search. Bit-exact with the previous transform (output identical); decode 4K 4:2:0 26 → 22 ms, 1080p 6.6 → 5.6 ms, 480p 1.08 → 0.90 ms, now at or below libjpeg-turbo on all three.
- **JPEG encoder rewrite**: the AC coder emitted a zero-run code every time sixteen zeros accumulated, even with nothing left in the block, so a block with a long zero tail spent up to three 11-bit ZRL codes before its 4-bit end-of-block; runs are now only spent when a coefficient follows, which alone removes the 22% size gap to libjpeg-turbo. The pipeline is rebuilt around MCU rows: RGB to YCbCr 16 pixels at a time with the fixed-point constants of `convertColor`, chroma downsampled with rounding in vectors, the forward DCT as libjpeg's exact integer transform on two blocks side by side in 16-lane vectors (the same paired multiply-add layout as the inverse), quantization by 16-bit reciprocals that equal `(|x| + d/2) / d` exactly (tested over every table value and the whole coefficient range), and a 64-bit bit packer that codes a block from a bitmask of its nonzero coefficients, one step per coded coefficient, with branch-free magnitude bits and a merged code-and-length table. Encode 4K 4:2:0 q90 92 → 20 ms, 1080p 22.9 → 4.9 ms, 480p 3.3 → 0.7 ms; libjpeg-turbo (Pillow) takes 18.3 / 4.4 / 0.6 ms and writes files within a few bytes of ours at the same PSNR in 4:4:4, 4:2:2 and 4:2:0. Quantization now rounds ties away from zero and chroma averaging rounds to nearest, as libjpeg does, so encoded bytes differ from the previous release at the same quality.
...
- **PNG encodes in parallel**: the filtered rows are deflated in chunks of ~256 KiB (whole rows, aligned to the adaptive filter's sample rate), each an independent `std.compress.flate` raw stream ended by a sync flush so the streams concatenate under one zlib header, with the adler32 combined from the chunks; rows are filtered in the same bands on `io`. The chunk size is fixed, so the bytes do not depend on the thread count. 4K RGB on 8 cores: 344 → 77 ms (Pillow/libpng: 1189 ms, 4 % smaller), 1024²: 52 → 17 ms; files grow 0.3 % (4K) to 0.7 % (1024²) from the per-chunk window reset. The adaptive filter now analyzes rows purely by position (every 8th row on tall images plus the first and last three), dropping the streak re-analysis, so chunks choose the same filters as a single sweep.
- **JPEG encodes in parallel across restart intervals**: `jpeg.encode` takes an `io: Io` and splits the scan into one band per CPU, each owning a run of restart segments: it fills, transforms and Huffman-codes its MCU rows into its own bit writer from fresh DC predictors, and the segments are appended with the RSTn markers between them, byte-identical to a single sweep. 4K 4:2:0 quality 90 on 8 cores: 15.6 → 5.0 ms (Pillow/libjpeg-turbo: 13.0 ms). Without restart markers (`.none`) there is one band and nothing changes.
- **Baseline JPEGs with restart intervals decode in parallel**: a DRI segment can be entropy-decoded from its RSTn marker with fresh DC predictors, so `decodeInto` splits the MCU rows into one band per CPU on the caller's `Io`, each starting at the restart segment holding its first MCU (re-decoding under one MCU row) and rendering through its own scratch. 4K 4:2:0 on 8 cores: 18.8 → 4.9 ms with one-row intervals, 19.3 → 5.8 ms with 4-MCU intervals; files without markers are unchanged (same instruction count), and the output is bit-identical to the single sweep. A damaged or truncated segment now zeroes only itself and decoding resumes at the next marker, on both paths alike.
- **Separable resize for f32 planes and interleaved struct pixels**: `resize` (and `scale`/`letterbox`) ran f32 images through the per-pixel 2-D kernel and split `Rgb`/`Rgba` into planes resized one after another. The fixed-point u8 tap tables now carry unit-gain float weights too, so f32 planes take the same two separable passes with an f32 accumulator plane, and struct pixels of u8 fields run through the u8 passes interleaved (the horizontal taps index pixels, every channel of each; the vertical pass is channel-agnostic). 4K → 1080p: f32 bilinear 19.9 → 8.7 ms serial, bicubic 48.8 → 10.0 ms, lanczos 108 → 18.3 ms (2.7 ms on the pool); Rgb bilinear 29.1 → 15.9 ms serial and 8.8 → 3.7 ms on the pool, lanczos 48.4 → 32.8 / 13.4 → 6.3 ms. u8 planes are unchanged (same cycle counts), struct pixels are bit-identical to the per-plane result, f32 within float rounding of the 2-D kernel.
- **Interleaved recursive Gaussian for struct pixels**: the IIR `gaussianBlur` also runs over the interleaved bytes instead of split planes: the row pass keeps one recursion per channel, stepping elements whose channel is fixed at comptime by grouping the transposed 16×16 blocks into lcm(16, channels) runs, and the column pass is channel-agnostic. Rgb σ=3 at 480×640: 2.26 → 1.63 ms serial, 1.36 → 0.70 ms on the pool; at 4K 27.4 → 16.5 ms on the pool (3.1× u8, from 5.9×). Bit-identical to the per-plane result.
- **Interleaved separable convolution for struct pixels**: `convolveSeparable` and the FIR `gaussianBlur` used to split an `Rgb`/`Rgba` image into planes, convolve them one after another and merge; they now run straight over the interleaved bytes, every byte a SIMD lane and the horizontal taps `channels` apart, so the split/merge traffic and the per-plane sequencing are gone. Rgb σ=3 at 4K: 55.9 → 49.9 ms serial and 13.5 → 9.5 ms on the pool (3.1× u8, from 3.6×/4.3×); horizontal Rgb motion blur at 480×640: 0.80 → 0.36 ms serial, 0.38 → 0.13 ms on the pool. Bit-identical to the per-plane result in every border mode.
...
### Changed
- **JPEG encodes write a restart marker per MCU row by default**: `EncodeOptions.restart_interval` is a `RestartInterval` (`.none`, `.{ .rows = n }`, `.{ .mcus = n }`) defaulting to one row, so files from `save`/`encode` (and Python) decode in parallel. Cost: ~0.1 % of the file at 4K and ~2 % more cycles single-threaded (same instruction count; a DC predictor reset and a marker resync per row); `.none` restores the old output byte for byte.
- **Codec `encode` takes an `io: Io`**: `jpeg.encode(T, io, allocator, image, options)` and likewise `png`, `bmp` and `gif`, so the codecs stay uniform behind `Image.save`; JPEG bands its scan on it, PNG converts its input on it, BMP and GIF are serial. Python is unchanged.
- **Decoding from bytes takes an `io: Io`**: `Image.loadFromBytes(io, allocator, bytes)`, every codec's `loadFromBytes(T, io, allocator, bytes, limits)` and `gif.loadAnimatedFromBytes`, and `jpeg.decodeInto`/`jpeg.toNativeImage`. JPEG uses it for the restart-interval bands, the other codecs for the output pixel conversion; `load(io, ...)` already had one. Python is unchanged.
- **`convert`, `convertInto`, `flipLeftRight`, `flipTopBottom` and `insert` take an `io: Io`** right after `self` (`image.convert(io, allocator, u8)`, `image.flipLeftRight(io)`, `canvas.insert(io, source, rect, angle, method, blend)`), the last per-pixel image operations without one, and run in row bands on it: at 4K, Rgb → u8 conversion 4.8 → 1.1 ms on the pool (serial unchanged), both flips 11.3 → 5.4 ms, and a rotated bilinear `insert` into a 1080p canvas 15.4 → 4.4 ms. `insert` instantiates its band per interpolation method so the per-pixel dispatch folds away; with a literal blend mode inlined into the caller the old loop was fully specialized and ran ~12 % fewer instructions serially. Codecs convert with `parallel.inline_io` (serial, as before); Python passes its pool.
