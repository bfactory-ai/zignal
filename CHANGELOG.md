...
- **Round joins for thick strokes**: `drawPolygon`, `drawRectangle`, the Bézier curves and `drawSplinePolygon` render widths above 1 as one joined stroke with round joins and caps, so corners are no longer notched and translucent strokes no longer double-blend at the segment caps.
